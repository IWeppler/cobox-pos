-- ---------------------------------------------------------------------------
-- `registrar_venta`: la cantidad de la cabecera acepta decimales.
--
-- Era `(p_venta->>'cantidad')::int`. `ventas.cantidad` ya es numeric(12,3)
-- desde 20260819175959, pero `->>` devuelve TEXTO, así que una venta con
-- 2,75 (0,750 kg de fiambre + 2 gaseosas) llegaba como '2.75' y
-- `'2.75'::int` es 22P02: la RPC entera falla y la VENTA SE CAE.
--
-- Hoy no se rompe nada porque los 4 negocios venden por unidad y la suma
-- siempre da entero. Se arregla ahora igual: es exactamente la misma clase de
-- bug que rompió la edición de productos, encontrado en la misma auditoría —
-- la que se rehizo con el regex corregido después de que el primero
-- (`\m...::int\M`) no matcheara ningún casteo, porque `\m` exige comienzo de
-- palabra y `:` no lo es.
--
-- Lo ÚNICO que cambia respecto de 20260816140000 es ese casteo.
-- ---------------------------------------------------------------------------
...
begin
  ...
  values (
    ...
    -- ERA ::int. Ver la cabecera del archivo.
