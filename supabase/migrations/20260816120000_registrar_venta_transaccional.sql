-- La venta se escribe en UNA transacción.
--
-- Hasta acá la cabecera, los pagos, los renglones, la deuda de cuenta corriente
-- y el espejo de stock eran seis escrituras sueltas, cada una en su propio
-- round-trip. Cualquiera podía fallar dejando a las anteriores hechas, y la de
-- los pagos ni siquiera revertía:
--
--   if (pagoError) return { error: ..., success: false };
--
-- Para cuando eso corría ya existían la fila de `ventas`, el movimiento de
-- cuenta corriente y el stock descontado. Quedaba una venta CONFIRMADA sin un
-- solo pago: el arqueo del turno no cierra, la mercadería salió del inventario
-- y la vendedora ve un error que la invita a reintentar — y el reintento
-- descuenta stock otra vez. El insert de `ventas_items` era peor todavía: no se
-- chequeaba, así que una venta podía quedar cobrada sin saber qué se vendió.
--
-- Qué queda ADENTRO y qué queda AFUERA:
--
-- Adentro va todo lo que solo tiene sentido junto — si la venta no existe, sus
-- pagos y sus renglones tampoco deben existir. Afuera quedan el descuento de
-- stock (`ajustar_stock_variante`) y el marcado de unidades serializadas
-- (`vender_unidades_serie`), que YA son atómicos por su cuenta y ya tienen su
-- camino de reversión en Node. Meterlos acá no agregaría garantía y sí
-- mezclaría dos cosas que fallan por motivos distintos: el stock falla porque
-- no hay mercadería (es una respuesta al usuario), esto falla porque algo se
-- rompió.
--
-- SECURITY INVOKER a propósito (es el default, se deja explícito): el
-- aislamiento por negocio tiene que seguir siendo la RLS del que llama. Una
-- función SECURITY DEFINER acá sería un agujero por el que se escribe en
-- cualquier negocio.
--
-- De paso quedan resueltas tres lecturas-y-después-escrituras que estaban en
-- Node y perdían actualizaciones concurrentes: el saldo del cliente, los usos
-- de la promoción y el espejo legacy de stock. Las tres pasan a ser deltas
-- (`columna = columna ± x`), que es lo único que sobrevive a dos cajas
-- operando a la vez.
